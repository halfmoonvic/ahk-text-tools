...
process {
    ...
    try {
        ...
        $source = if ($hasPipeline) { [string]::Join([Environment]::NewLine, $parts.ToArray()) }
            elseif ($explicitText) { $Text }
            elseif ($hasInputFile) { [IO.File]::ReadAllText((Resolve-EntryFilePath $InputFile), $utf8) }
            else {
                $reader = [IO.StreamReader]::new([Console]::OpenStandardInput(), $utf8, $true, 1024, $true)
                try { $reader.ReadToEnd() } finally { $reader.Dispose() }
            }
        if ([string]::IsNullOrWhiteSpace($source)) { throw 'no input text provided' }
        $outputPath = if ($hasOutputFile) { Resolve-EntryFilePath $OutputFile } else { '' }
        ...
        if ($translateExitCode -ne 2) { $translateExitCode = 1 }
        ...
        if ($translateExitCode -eq 130) {
            ...
            for ($argumentIndex = 0; $argumentIndex -lt $hostArguments.Length - 1; $argumentIndex++) {
                if ($hostArguments[$argumentIndex] -ieq '-File') {
                    ...
                    if ($hostScript -ieq $PSCommandPath) { [Environment]::Exit(130) }
                    break
                }
            }
        }
