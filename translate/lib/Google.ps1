...
# HTTP API directly, so most of the work here is building a clean child
# environment and making sure the process tree dies with us.
...
function Get-GitBash {
    ...
    foreach ($key in @('HKCU:\Software\GitForWindows', 'HKLM:\SOFTWARE\GitForWindows')) {
        ...
        if ($null -ne $installation) {
            $candidates += Join-Path $installation.InstallPath 'bin\bash.exe'
        }
    }
...
#   Translate Text via the vendored Google Shell script and publish the result
#   to OutputFile, or to stdout when OutputFile is empty.
#
#   Output is written once, after the child exits, so a partial translation is
#   still delivered when a later line fails. The finally block kills the whole
#   process tree: bash spawns gawk and curl, which would otherwise outlive a
#   cancellation.
# ---------------------------------------------------------------------------
function Invoke-GoogleTranslate([string] $Text, [string] $Target, [string] $OutputFile, $Common, [Threading.CancellationToken] $CancellationToken) {
    ...
    try {
        ...
        foreach ($key in @($process.StartInfo.EnvironmentVariables.Keys)) {
            if ($key -match '^(BASH_ENV|ENV|SHELLOPTS|BASHOPTS|TRANS_.*)$') {
                $process.StartInfo.EnvironmentVariables.Remove($key)
            }
        }

        $process.StartInfo.EnvironmentVariables['PATH'] =
            (Join-Path (Split-Path (Split-Path $bash -Parent) -Parent) 'usr\bin') + ';' + $env:PATH
        ...
        if ($errorText -match '(?m)^\[ERROR\]') {
            throw "Google Shell reported an error: $($errorText.Trim())"
        }

        if ([string]::IsNullOrWhiteSpace($result)) {
            throw 'Google Shell returned an empty translation'
        }
    } finally {
        if ($null -ne $process -and $started) {
            if (-not $process.HasExited) {
                ...
                try {
                    ...
                    $killer.WaitForExit()
                    [void]$killOut.GetAwaiter().GetResult()
                    [void]$killErr.GetAwaiter().GetResult()
                } finally {
                    $killer.Dispose()
                }
            }
